...
1. "It has some kind of type specifier (like s or i), followed by a colon, followed by the actual data, followed by a semicolon."
2. The basic types available are N for NULL, b for boolean, i for integer, f for floating-point, and s for string.
3. Things can be bundled up and refered to as a whole, so the compound types available are a for array, and O for object.
4. When things are bundled up, if the count number provided is half of the number of things in that bundle (determined by the number of semicolons), pairs of a key and a value are assumed.<br>
Based on above, whatever payload is fed into `unserialize()`, it *at most* can come out as:<br>
1. a piece of data e.g. 'Success', 'users.txt'.
2. a bundle of pieces of data e.g. ['Success', 'users.txt'].
3. a bundle of pairs of data tied together i.e. key-value pairs or variables e.g. ['user_file'=>'users.txt', 'data'=>'Success']<br>
